...
PlayState.initMap = function () {
	...
	this.isDungeon = Math.random()<0.5;
	if(this.isDungeon) {
...
PlayState.initEntities = function () {
	...
	for (var e = 0; e < this.numberOfActors; e++) {
		...
		if(e == 0) {
			...
			// let's make some simple, smart and random enemies
			var rand = Math.random();
			if(rand<1/3) {
...
PlayState.initScreen = function() {
	...
	// we have 6 types of tiles in our tile map
	for(var t=0; t<=6; t++) {
...
PlayState.tryToDig = function(actor, dir) {
	...
	if(this.isDungeon) {
		mapLayer.setTile(destX,destY,3+1);
	} else {
